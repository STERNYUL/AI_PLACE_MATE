...
**개정:** v1.2 (2026-08-25) — 태스크 축약(99→46) 및 W0 치명 5건 결정 반영
**총 태스크:** 99건 (추적 단위) · **46건** (실행 단위 — 부록 F) — SPEC 11 · MOCK 4 · FR 52 · NF 12 · QA 14 · UX 6
...
| **v1.2** | **축약 + 결정 반영** — 실행 단위 99→46 (부록 F) / W0 치명 5건 결정 반영 및 차단 라벨 17건 해제 (부록 G) | `docs/task-consolidation-plan.md` · `docs/W0-decisions.md` |
...
**개정일:** 2026-08-25 · **결과:** 실행 단위 **99 → 46** (54% 감축) · **원장 Task ID 99개 전량 보존**

### F-1. 축약의 성격

**병합은 실행 단위를 줄이는 것이고 추적 단위는 그대로다.**
각 병합 이슈 본문에 원장 Task ID 체크리스트가 남으므로 SRS 조항까지의 추적이 끊기지 않는다.

| 축 | 변화 |
| --- | --- |
| 실행 단위 (GitHub 이슈) | 93 → **46** |
| 추적 단위 (원장 Task ID) | 99 → **99 (불변)** |
...
| --- | --- |
| 1 | 같은 도메인 + 같은 담당 조직 |
| 2 | 합계 복잡도가 H를 넘지 않는다 (`H+H` 금지) |
| 3 | `Blocks` 5개 이상은 단독 유지 |
| 4 | **차단 안건이 다른 태스크는 묶지 않는다** — 막힌 것과 안 막힌 것을 묶으면 둘 다 막힌다 |

**병합 금지 8건** — `SPEC-000`(#1) · `SPEC-007`(#8) · `FR-021`(#41) · **`FR-035`(#48)** · `FR-049`(#85) · `NF-001`(#70) · `NF-003`(#71 그룹) · `QA-008`(#63)

### F-3. 병합 매핑 — 46건

| 대표 이슈 | 신규 ID | 원장 Task ID | 개수 | 흡수한 이슈 |
| --- | --- | --- | --- | --- |
| #1 | `SPEC-A` | `SPEC-000` | 1 | **단독** |
| #2 | `SPEC-B` | `SPEC-001` · `SPEC-002` · `SPEC-003` | 3 | #3, #4 |
| #5 | `SPEC-C` | `SPEC-004` · `SPEC-005` | 2 | #6 |
| #7 | `SPEC-D` | `SPEC-006` · `SPEC-010` | 2 | #11 |
| #8 | `SPEC-E` | `SPEC-007` | 1 | **단독** |
| #9 | `SPEC-F` | `SPEC-008` · `SPEC-009` | 2 | #10 |
| #17 | `MOCK-A` | `MOCK-001` · `MOCK-002` | 2 | #18 |
| #19 | `MOCK-B` | `MOCK-003` · `MOCK-004` | 2 | #20 |
| #12 | `DB-A` | `FR-001` · `FR-002` · `FR-003` · `FR-004` · `FR-005` | 5 | **단독** |
| #13 | `DB-B` | `FR-006` · `FR-007` | 2 | #14 |
| #15 | `DB-C` | `FR-008` · `FR-009` | 2 | **단독** |
| #16 | `DB-D` | `FR-010` · `FR-011` | 2 | **단독** |
| #33 | `AUD-A` | `FR-012` · `FR-013` · `FR-014` | 3 | #34, #35 |
| #36 | `AUD-B` | `FR-015` | 1 | **단독** |
| #21 | `AUD-C` | `FR-016` · `FR-017` · `FR-018` | 3 | #37, #38 |
| #39 | `CMP-A` | `FR-019a` · `FR-020` · `FR-025` | 3 | #40, #45 |
| #41 | `CMP-B` | `FR-021` | 1 | **단독** |
| #42 | `CMP-C` | `FR-022` · `FR-023` · `FR-024` | 3 | #43, #44 |
| #22 | `CMP-D` | `FR-019b` · `FR-026` · `FR-027` · `FR-028` | 4 | #46, #47, #31 |
| #23 | `ADS-A` | `FR-029` · `FR-030` · `FR-031` | 3 | #24, #25 |
| #26 | `ADS-B` | `FR-032` · `FR-033` | 2 | #49 |
| #27 | `ADS-C` | `FR-034` · `FR-036` | 2 | #28 |
| #48 | `ADS-D` | `FR-035` | 1 | **단독** |
| #55 | `ADS-E` | `FR-037` · `FR-038` | 2 | #50 |
| #51 | `TRK-A` | `FR-039` · `FR-040` | 2 | #52 |
| #29 | `TRK-B` | `FR-041` · `FR-042` · `FR-043` | 3 | #30, #53 |
| #54 | `TRK-C` | `FR-044` · `FR-045` | 2 | #32 |
| #82 | `INT-A` | `FR-046` · `FR-047` | 2 | #83 |
| #84 | `INT-B` | `FR-048` | 1 | **단독** |
| #85 | `INT-C` | `FR-049` | 1 | **단독** |
| #86 | `INT-D` | `FR-050` · `FR-051` | 2 | #87 |
| #70 | `INF-A` | `NF-001` | 1 | **단독** |
| #71 | `INF-B` | `NF-003` · `NF-005` | 2 | #74 |
| #72 | `INF-C` | `NF-002` · `NF-004` | 2 | #73 |
| #75 | `INF-D` | `NF-006` · `NF-007` | 2 | #76 |
| #78 | `SEC-A` | `NF-008` · `NF-009` | 2 | #79 |
| #80 | `SEC-B` | `NF-010` · `NF-012` | 2 | #81 |
| #56 | `TST-A` | `QA-001` · `QA-002` | 2 | #57 |
| #58 | `TST-B` | `QA-003` · `QA-006` · `QA-007` | 3 | #61, #62 |
| #59 | `TST-C` | `QA-004` · `QA-005` | 2 | #60 |
| #63 | `TST-D` | `QA-008` | 1 | **단독** |
| #64 | `TST-E` | `QA-009` · `QA-010` | 2 | #65 |
| #66 | `TST-F` | `QA-011` · `QA-012` · `QA-013` · `QA-014` · `NF-011` | 5 | #67, #68, #69, #77 |
| #88 | `UX-A` | `UX-001` · `UX-004` | 2 | #91 |
| #89 | `UX-B` | `UX-002` · `UX-003` | 2 | #90 |
| #92 | `UX-C` | `UX-005` · `UX-006` | 2 | #93 |

**흡수되어 Close된 이슈 47건** — `superseded` 라벨 부여 + 통합 대상 참조 코멘트 첨부.
크리티컬 패스 이슈 번호(`#1` `#12` `#48` `#55` `#88`)는 대표로 유지되어 기존 문서 참조가 유효하다.
...
`NF-011`(enum 확장성 리뷰)은 검증 성격이므로 `TST-F`(#66)로 이관했다.

### F-5. 감축이 아니라 정합성 개선인 병합 3건

| 병합 | 이유 |
| --- | --- |
| `SPEC-D` = SPEC-006 + 010 | **스키마 통일이 목적**이므로 분리가 오히려 위험했다 |
| `SPEC-F` = SPEC-008 + 009 | **멱등키 규약을 공유**해야 하므로 분리하면 규칙이 갈린다 |
| `ADS-A` = FR-029 + 030 + 031 | **같은 조건 결합 규칙**을 세 사람이 다르게 해석할 위험이 있었다 |

---

## 부록 G · W0 치명 5건 결정 반영 (v1.2)

전문은 `docs/W0-decisions.md`. 아래는 부록 D에 대한 해소 결과다.

| 안건 | 결정 요지 | 부록 D 해소 |
| --- | --- | --- |
| **D-01** | Bearer 토큰 + 자원 소유자 검증 · **타인 자원은 `404`**(존재 누설 방지) · 역할 3종(MVP 2종) | 인증 방식 · 인가 규칙 **해소** |
| **D-02** | **조건부 원자 갱신** · 허용 초과 **0%** · 동기 소진 감지 · 동점은 소진율→해시 | 예산 동시성 · 동점 규칙 **해소** |
| **D-03** | **eCPM 환산** · 3단 계단식 CTR(표본 1,000건 문턱) · 과금 3종 유지 | 선택 기준 **해소** |
| **D-04** | `UNKNOWN` 3종 추가(격자 36→**80칸**) · 프로파일 물리 삭제 예외 · 보존 기간 4종 | `UNKNOWN` 부재 · 파기 경로 · 보존 기간 **해소** / 소득 근거는 **법무 대기** |
| **D-05** | 측정 5항목 확정 · 성능 예산 배분표 · 단계별 타임아웃 20/20/15ms · **G1 200ms / G2 100ms** | 측정 정의 **해소** / 목표치는 실측 후 확정 |

### G-1. 라벨 해제 결과

| 라벨 | 이전 | 이후 |
| --- | --- | --- |
| `blocked-d01` | #7 #78 #80 | **해제** |
| `blocked-d02` | #42 #48 | **해제** |
| `blocked-d03` | #27 | **해제** |
| `blocked-d05` | #64 #72 | **해제** |
| `blocked-d04` | #13 #85 | → **`pending-legal`** (소득 차원 활성화만 대기) |
| `blocked-ac` | #63 | 유지 — 어트리뷰션 윈도우 등 부록 D 잔여 항목 |

**차단 이슈 20건 중 17건이 해제되고 2건이 조건부, 1건이 유지된다.**

### G-2. 착수를 막지 않는 우회 설계 2건

| 안건 | 우회 |
| --- | --- |
| D-04 소득 근거 | `income_segmentation_enabled = false` 기본값. 분류기·스키마·API는 80칸 기준 구현, 운영은 20칸(연령 5 × 지역 4)으로 시작. 법무 승인 시 설정 하나로 활성 |
| D-05 목표치 | **G1 200ms 주간 게이트**로 개발 진행, **G2 100ms 인수 게이트**에서 실측 데이터로 재협상 판단 |

### G-3. SRS 수정 필요 조항 7개

| 조항 | 변경 |
| --- | --- |
| REQ-NF-001 | 측정 5항목 · 2단계 게이트 · 단계별 타임아웃 |
| REQ-NF-002 | 응답 시간 조건 추가 · 유실률 0.1% |
...
**SRS v1.1 개정이 필요하다.** `docs/W0-decisions.md`를 근거 문서로 첨부하면 승인 절차가 짧아진다.

---

*원장 v1.2 · 2026-08-25 · 실행 단위 46 · 추적 단위 99*
