...
| REQ-NF-002 | Should | FR-008, FR-040, NF-004, NF-005 | 4 | ✅ |
| REQ-NF-003 | Must | NF-006, NF-007 | 2 | ✅ |
| REQ-NF-004 | Must | NF-008, NF-009, NF-010 | 3 | ✅ |
| REQ-NF-005 | Could | FR-005, NF-011 | 2 | ✅ |
...
### B-3. 역방향 커버리지 — 태스크 → SRS 근거 (77 / 77 · 고아 태스크 0)
...
| QA-001 ~ QA-009 | 9 | §5 테스트 케이스 ID(TC-FUNC-001~008, TC-NF-001) · 검증 방식 열 | 0 |
...
### B-4. 검증 커버리지 — 격차 존재 (TC ID 기준 9 / 13 · 69%)

| 요구사항 | §5의 TC ID | 검증 태스크 | 상태 |
| --- | --- | --- | --- |
| REQ-FUNC-001 ~ 008 | TC-FUNC-001 ~ 008 | QA-001 ~ QA-008 | ✅ 지정됨 |
| REQ-NF-001 | TC-NF-001 | QA-009 | ✅ 지정됨 |
| REQ-NF-002 | **없음** | NF-005 (검증 방식 열에서 도출) | ⚠️ TC ID 미지정 |
| REQ-NF-003 | **없음** | NF-007 (검증 방식 열에서 도출) | ⚠️ TC ID 미지정 |
| REQ-NF-004 | **없음** | NF-010 (검증 방식 열에서 도출) | ⚠️ TC ID 미지정 |
| REQ-NF-005 | **없음** | NF-011 (검증 방식 열에서 도출) | ⚠️ TC ID 미지정 |

```mermaid
flowchart LR
  subgraph OK["추적 사슬 완결 — 9건"]
    direction LR
    R1["REQ-FUNC-001~008<br/>REQ-NF-001"] --> M1["§5 모듈·클래스"] --> T1["TC-FUNC-001~008<br/>TC-NF-001"] --> Q1["QA-001~009"]
  end
  subgraph NG["추적 사슬 단절 — 4건"]
    direction LR
    R2["REQ-NF-002<br/>REQ-NF-003<br/>REQ-NF-004<br/>REQ-NF-005"] --> M2["§5 미등재"] -.->|"TC ID 없음"| T2["미지정"]
    R2 --> V2["검증 방식 열만 존재<br/>→ NF-005·007·010·011로 대체 도출"]
  end

  classDef ok fill:#eaf4ee,stroke:#1e7a55,color:#124434
  classDef ng fill:#fdeceb,stroke:#bc3226,stroke-width:2px,color:#6f1a13
...
**조치 필요.** REQ-NF-002·003·004·005는 §5 추적성 매트릭스에 행 자체가 없어 담당 모듈과 테스트 케이스 ID가 지정되지 않았다.
확장성·가용성·보안·유지보수성 전부가 여기 해당한다. NF-005·007·010·011은 §4.2 검증 방식 열만을 근거로 도출한 것이므로,
**TC-NF-002 ~ TC-NF-005 신설과 §5 갱신이 착수 전 선행 과제**다.
...
| 역방향 — 태스크 근거 추적 | 77 | 77 | 100% | ✅ 통과 (고아 0) |
| 검증 — TC ID 기준 | 13 | 9 | 69% | ⚠️ **격차 4건** |
| 인수 판정 가능성 — 부록 D 기준 | 13 | 8 | 62% | ⚠️ **미정의 13항목** |
