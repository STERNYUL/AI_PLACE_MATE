...
graph LR
  ...
  EVDB["EVD-B · 문장 생성·판정형 차단<br/>M · 1주"]:::crit
...
**경로가 9마디이고 그중 7개가 H다 (`EVD-B`·`CLI-E`만 M).** 원장 §6이 경고한 *"H 비중 58%"* 가 임계 경로에서 그대로 나타난다.
...
> **단축 후보는 근거 3연쇄다.** `EVD-A`~`C`가 5주를 차지한다. 세 건을 병렬화하려면
...
| `EVD-A` → `EVD-B` → `EVD-C` | 5주 | 검증기 → 문장 생성 → 카드. **§2.3의 단축 후보** |
