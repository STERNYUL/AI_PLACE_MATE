...
| `MOCK-002` Top-3 6시나리오 | `lib/fixtures/top3.json` | 시나리오 이름이 그대로 `MOCK-002`의 스위치 키가 된다 |
| `MOCK-003` 근거 누락 4종 | `lib/fixtures/evidence-missing.json` | 동일 |
| `MOCK-004` 신선도 3상태 + 경계 3종 | `lib/fixtures/freshness.json` | 동일 |
