...
| 11 | [`P3a`](P3a-tests-search.md) · [`P3b`](P3b-tests-evidence.md) · [`P3c`](P3c-tests-phase2-nf.md) | `TEST-001`~`014` (14) | 0 (1) · 1 (8) · 1말 (2) · **2** (3) |
| 12 | [`P4a-infra.md`](P4a-infra.md) | `IN-A`~`G` (7) | 0 (4) · 1 (3) |

**총 81건** — 원장 50건 + `SPEC-` 9 + `MOCK-` 8 + `TEST-` 14.
정본 §3·§2가 최종 구성으로 기록한 **원장 50 + 신규 31 = 81** 과 일치한다.
...
| **Phase 1** | 24 | — | 5 (`002`~`005` `007`) | 8 (`TEST-002`~`008` `014`) | **37** |
...
| --- | --- |
| Phase 1 기간 | **8주** |
| Phase 1 태스크 (확장 후) | **37건** |
| 그중 복잡도 H | **14건** |
| `TEST-014` (비기능 32건 단일 태스크) | 포함 |

**`TEST-014` 분할(`P3c` 제안)을 적용하면 40건**이 된다.
...
| `TEST-014`의 Phase 배치 | **`phase-1`인데 선행 `PRV-A`·`PRV-B`·`RSV-C`가 `phase-1-late`** ⚠ |

**3건이 남아 있다.** 앞의 2건은 원장 부록 C에 기록됐고 PM 확정 대기 중이다.

> `TEST-014`는 Phase 경계를 넘지는 않는다(1 → 1말). 다만 **Phase 1 안에서 판정을 끝낼 수 없다** —
> 개인정보·결제 검증 항목은 Phase 1 말 산출물이 나와야 실행된다. `P3c`의 분할 제안과 함께 정리해야 한다.
...
| 17 | **`TEST-014` 분할 여부** (`REQ-NF` 32건 단일 태스크) | 81 → 84건 | PM |
