...
**웨이브:** 제작 순서 13번 (최종) · **전 Phase**
**근거:** `SRS-ai-place-v1.0.md` v1.9 · **원장** v1.1 · 본문 문서 9종
**정본** [`task-extraction-assessment-aiplace.md`](../task-extraction-assessment-aiplace.md) Step 4

> **이 문서는 새 이슈를 만들지 않는다.** 앞의 9개 문서에서 드러난 사실을 모아
...
| # | 문서 | 이슈 | Phase |
| --- | --- | --- | --- |
| 1 | [`P1a-contracts.md`](P1a-contracts.md) | `SPEC-001`~`009` (9) | 0 |
| 2 | [`P1b-data.md`](P1b-data.md) | `IDX-A`~`E` (5) | 0~1 |
| 3 | [`P1c-mock.md`](P1c-mock.md) | `MOCK-001`~`008` (8) | 0 |
| 4 | [`P2a-search.md`](P2a-search.md) | `SRC-A`~`D` (4) | 1 |
| 5 | [`P2b-evidence.md`](P2b-evidence.md) | `EVD-A`~`D` (4) | 1 |
| 6 | [`P2c-tracking.md`](P2c-tracking.md) | `TRK-A`~`E` (5) | 0~1 |
| 7 | [`P2d-reservation-privacy.md`](P2d-reservation-privacy.md) | `RSV-A`~`D` `PRV-A`~`B` (6) | 1말 |
| 8 | [`P2e-client.md`](P2e-client.md) | `CLI-A`~`E` (5) | 1~1말 |
| 9 | [`P2f-ux.md`](P2f-ux.md) | `UX-A`~`F` (6) | 0~1말 |
| 10 | [`P2g-phase2.md`](P2g-phase2.md) | `MCH-A` `AGT-A~C` `CLI-F~G` `UX-G~H` (8) | **2** |
| 11 | [`P3a`](P3a-tests-search.md) · [`P3b`](P3b-tests-evidence.md) · [`P3c`](P3c-tests-phase2-nf.md) | `TEST-001`~`014` (14) | 0~2 |
| 12 | [`P4a-infra.md`](P4a-infra.md) | `IN-A`~`G` (7) | 0~1 |

**총 81건** — 원장 50건 + `SPEC-` 9 + `MOCK-` 8 + `TEST-` 14.
정본 §3이 예고한 *"111 → 약 149"* 는 축약안 확정으로 **50 → 81** 이 됐다.
...
| --- | --- | --- | --- | --- | --- |
| **Phase 0** | 10 | 9 | 8 | 1 (`TEST-001`) | **28** |
| **Phase 1** | 24 | — | — | 8 (`TEST-002`~`008` `014`) | **32** |
| **Phase 1 말** | 8 | — | — | 2 (`TEST-009` `010`) | **10** |
| **Phase 2** | 8 | (`SPEC-010` 신설 시 +1) | — | 3 (`TEST-011`~`013`) | **11** |
| **합계** | 50 | 9 | 8 | 14 | **81** |

### 2.2 Phase 0가 10건 → 28건이 됐다

**2주에 28건은 성립하지 않는다.** 그러나 성격을 보면 다르다.

| 성격 | 건수 | 비고 |
| --- | --- | --- |
| **계약 문서** (`SPEC-`) | 9 | 코드가 아니다. 합의가 산출물 |
| **목 서버** (`MOCK-`) | 8 | 계약에서 파생. 자동 생성 가능 |
| **인프라·데이터** | 10 | 실제 구축 |
| **검증** | 1 | 평가셋 구축 포함 |

**`SPEC-`은 2주 안에 **쓰는** 것이 아니라 **정하는** 것이다.**
`P1a`가 지적한 대로, 계약이 얼어야 Phase 1의 병렬 작업이 가능하다.
**Phase 0의 실질 병목은 인프라 10건이 아니라 계약 합의 9건이다.**

### 2.3 Phase 1의 H 13건 문제 — 미해소
...
| --- | --- |
| Phase 1 기간 | **8주** |
| Phase 1 태스크 (확장 후) | **32건** |
| 그중 복잡도 H | **13건** |
| `TEST-014` (비기능 32건 단일 태스크) | 포함 |

**`TEST-014` 분할(`P3c` 제안)을 적용하면 35건**이 된다.
...
**Phase 2 11건을 전부 제거해도 Phase 1이 완결되는가?**
...
**2건이 남아 있다.** 둘 다 원장 부록 C에 기록됐고 PM 확정 대기 중이다.

---

## 3. 착수 전 확정해야 할 항목 — 전 문서 통합

본문 10개 문서에서 드러난 미정 항목을 **성격별로** 모았다.
...
| 17 | **`TEST-014` 분할 여부** (32건 단일 태스크) | 50 → 53건 | PM |
