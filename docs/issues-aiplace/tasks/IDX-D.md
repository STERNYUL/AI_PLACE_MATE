...
- [x] **확인 주체 필드 정의** — **열거형 4종.** 표시명은 `lib/evidence` 매핑 (확정 · Grill S3-T2)
- [x] **`SPEC-008`의 `STALE` 유효성 판정 반영** — 노출 + 경고 병기 (세션 2 T2)

### `verified_by` — 열거형 4종 (확정 · Grill S3-T2 · `IDX-A1`에서 언다)

| 값 | 언제 | 출처 |
| --- | --- | --- |
| `MERCHANT` | 매장이 콘솔에서 저장 | `UX-H` — *"매장이 스스로 확인 주체가 된다"* |
| `INTERNAL_SURVEY` | 내부 조사로 적재 | `IDX-C` 초기 적재 300건 · 세션 1 T7 fixture |
| `USER_REPORT` | 사용자 신고를 거쳐 재확인 | `EVD-D` 재확인 큐 |
| `OPERATOR` | 운영자가 직접 수정 | 운영 경로 |

**표시 문구는 스키마에 넣지 않는다.** `lib/evidence`의 매핑 함수 한 곳이 열거값 → 화면 문구를 만든다.

**자유 텍스트로 두지 않는 이유가 둘이다.**

| | 내용 |
| --- | --- |
| **사칭 방지** | 매장 확인이 아닌데 `"사장 확인"` 이라고 쓰는 것을 **스키마가 구조적으로 막는다** (세션 1 T7 가드) |
| **판정형 grep 범위** | 확인 주체는 근거 4항목이라 화면에 그대로 나간다. 자유 텍스트면 §8.3 규칙 3 검사가 **DB 값까지 쫓아가야 한다** |

**네 경로가 서로 다른 주체를 만든다** — 콘솔 저장(`UX-H`) · 초기 적재(`IDX-C`) · 재확인(`EVD-D`) · 운영 수정. 자유 텍스트면 같은 것을 세 가지 표기로 쓰게 된다.
