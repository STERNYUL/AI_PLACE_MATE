...
| **1.2** | **2026-08-25** | **노출 경매 기능 전면 취소, 순차 노출로 전환** — REQ-FUNC-005 전면 개정, §1.2 범위 · §3 서비스 설명 · §5 구현 클래스 · §6.2 enum 주석 · §6.3 규칙 4 갱신, eCPM 환산 및 예상 CTR 추정을 §7.1로 이관 | 사업 결정 |

> **v1.1 개정 성격** — 신규 기능 추가가 아니라 **검증 불가능했던 인수 기준을 판정 가능하게 만든 개정**이었다.
> v1.0의 13개 조항 중 인수 기준을 온전히 쓸 수 있는 것은 2건이었다(`TASKS-adtech-mvp-v1.0.md` 부록 E).
>
> **v1.2 개정 성격 — 이것은 사업 범위 축소다.** 노출 단위 경매를 수행하지 않고 후보를 균등 순환 노출한다.
> **수익 최적화를 의도적으로 포기하는 결정**이며, 그 대가와 새로 생기는 문제는 아래에 명시했다.

### v1.2 — 순차 노출 전환의 대가

경매를 없애면 구현이 크게 단순해지지만 **세 가지 문제가 새로 생긴다.** 착수 전에 인지해야 한다.

| # | 문제 | 설명 |
| --- | --- | --- |
| **1** | **노출당 수익 하락** | 단가가 높은 캠페인과 낮은 캠페인이 **같은 횟수** 노출된다. eCPM 비교를 하지 않으므로 지면의 수익 잠재력을 쓰지 않는다. 이것이 이 결정의 직접 비용이다 |
| **2** | **예산 규모별 불형평** | 균등 순환은 **노출 수를 균등하게** 만들지 뿐 예산 소진을 균등하게 만들지 않는다. 일 예산 1,000만 원 광고주와 10만 원 광고주가 같은 노출을 받으면 → 큰 쪽은 예산을 다 못 쓰고, 작은 쪽은 하루 일찍 소진된다 |
| **3** | **페이싱 부재와의 결합** | 예산 페이싱이 §7 이관 상태(v1.0부터)라, 문제 2가 오전 집중 소진과 겹쳐 증폭된다 |

**문제 2가 계약 리스크다.** 큰 광고주가 "예산을 다 못 썼다"고 문제 제기할 수 있다.
**예산 비례 가중 순환**(예산이 큰 캠페인에 더 많은 순번 배정)이 대안이나, 그것은 사실상 가치 기반 배분이라
"경매를 없앤다"는 이번 결정의 취지와 충돌한다. **균등 순환으로 시작하고 문제 2가 실제로 발생하면 재검토**하는 것을 권한다.
...
- **순차 노출**을 포함한 3단계 폴백 광고 제공 *(v1.2 — 입찰 기반 최적화에서 변경)*
- 실시간 성과 로깅 및 어트리뷰션
...
- **내부 마이크로서비스**
    ...
    - Ad Serving Engine : 순차 노출을 포함한 3단계 폴백 추천
...
| **REQ-FUNC-005** | **순차 노출 및 캠페인 선택** | 설계 문서 5.2<br>+ D-03 결정(v1.2 전면 개정) | Must Have | Functional | 1) 순환 균등성 테스트<br>2) 예산 동시성 테스트<br>3) 슬롯 중복 배제 검증 | 타게팅 조건과 예산 잔액을 만족하는 후보 중 **당일 노출 수가 가장 적은 캠페인**을 선택해야 한다. 노출 수가 동일한 후보가 복수인 경우 `requestId` 해시로 결정적으로 선택한다. **입찰가와 과금 방식은 선택에 관여하지 않으며 과금 계산에만 사용한다.** 슬롯이 복수인 경우 동일 규칙을 반복 적용하되 하나의 지면에서 같은 캠페인을 중복 선택하지 않는다 | **Approved** | 개발 엔지니어 |
...
| REQ-FUNC-005 | Ad Serving Engine | **SequentialAdSelector** *(v1.2 — YieldOptimizer 대체)* | TC-FUNC-005 |
...
// 캠페인 타게팅 설정
// v1.2 — BiddingStrategy 는 **과금 계산 전용**이다. 캠페인 선택에는 관여하지 않는다 (REQ-FUNC-005).
...
4. **순차 노출**: 타게팅 조건과 예산 잔액을 만족하는 후보를 **당일 노출 수 오름차순으로 순환 노출**한다. 노출 수가 동일하면 `requestId` 해시로 결정적으로 선택한다. **입찰가는 선택에 관여하지 않는다** — 과금 계산에만 사용한다 *(v1.2 — v1.1의 eCPM 환산 기반 수익 최적화를 전면 대체)*
...
### 7.1 경매 기반 수익 최적화 및 RTB 연동

**v1.2에서 이 절로 이관된 항목**

- **eCPM 환산 기반 캠페인 선택** — CPM은 입찰가 그대로, CPC는 `입찰가 × 예상CTR × 1000`, CPA는 `입찰가 × 예상CVR × 1000`으로 환산해 최댓값을 선택
- **예상 CTR·CVR 추정** — 캠페인×세그먼트 실적 → 캠페인 평균 → 기본값의 계단식 규칙
- **입찰가 동점 처리 규칙**

> **이관 근거** — MVP는 노출 단위 경매를 수행하지 않고 **순차 노출**(REQ-FUNC-005)로 지면을 배분한다.
> 경매 도입은 아래 RTB 연동과 함께 이루어지는 것이 자연스럽다. 두 항목을 한 절로 묶었다.
