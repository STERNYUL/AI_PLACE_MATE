...
> 예시 SRS에는 대응 절이 없다. PRD §1.1~§1.2의 Pain·실패 KPI·목표가 요구사항의 **출처**이므로 29148의 이해관계자 요구사항 계열로 확장했다.
...
> 예시 SRS에는 대응 절이 없다. 29148 부속서의 Assumptions and Dependencies 항목으로 확장했다.
...
> 예시 SRS 2절은 내부 역할만 다룬다. PRD §2의 페르소나·JTBD가 요구사항의 출처이므로 사용자 클래스 절로 확장했다.
...
> 예시 SRS에는 대응 절이 없다. PRD §2.2의 여정 도식과 이탈 분기를 단계 표로 치환해 확장했다.
...
> 예시 SRS에는 대응 절이 없다. PRD §4.2 의존성과 §4.3 규모·스프린트 분해를 표로 치환해 확장했다.
...
> 예시 SRS에는 대응 절이 없다. PRD §1.3의 KPI 체계를 요구사항에 역방향 연결해 확장했다.
...
> 예시 SRS에는 대응 절이 없다. 요구사항별 검증 방식(4.1·4.2)의 집합 계획이 필요하므로 29148 §4 Verification 계열로 확장했다.
...
> 예시 SRS에는 대응 절이 없다. PRD §7.2의 리스크와 ADR이 요구사항의 존립 조건을 규정하므로 독립 절로 확장했다.
...
> 예시 SRS에는 대응 절이 없다. PRD §9의 확보 근거를 요구사항의 출처 추적용으로 부록에 배치했다.
...
- **재방문·LTV 어트리뷰션**: 현재는 7·30일 재방문까지 측정한다. 세션 단위 결정을 이용자 생애 가치로 연결하는 모델을 도입한다
